...
def generate_toc_from_markdown(markdown: str) -> str:
    """Generate table of contents HTML from markdown headings (h2 and h3 only)."""
    ...
    for line in lines:
        ...
        if h2_match:
            ...
            # Remove markdown formatting like **bold**, *italic*, `code`
            clean_text = strip_inline_md(heading_text)
            # Generate anchor ID (simple version: lowercase and replace spaces with hyphens)
            # The actual anchor will be auto-generated by marked.js with same logic
            anchor = re.sub(r'[^\w\u4e00-\u9fff]+', '-', clean_text.lower()).strip('-')
            toc_items.append(f'            <a href="#{anchor}" class="toc-item">{html_lib.escape(clean_text)}</a>')
            continue
        ...
        if h3_match:
            ...
            clean_text = strip_inline_md(heading_text)
            anchor = re.sub(r'[^\w\u4e00-\u9fff]+', '-', clean_text.lower()).strip('-')
            toc_items.append(f'            <a href="#{anchor}" class="toc-item toc-item--h3">{html_lib.escape(clean_text)}</a>')
            continue

    if not toc_items:
        return '            <a href="#" class="toc-item">暂无目录</a>'
...
def apply_template(template_html: str, *, title: str, excerpt: str, date_str: str, read_text: str, cat: str, markdown: str) -> str:
    ...
    toc_html = generate_toc_from_markdown(markdown)
    out = re.sub(
        r'(<nav class="toc-body" id="toc">)\s*.*?\s*(</nav>)',
        rf'\g<1>\n{toc_html}\n          \g<2>',
        out,
        count=1,
        flags=re.S
...
def inject_template_elements(
    ...
    *,
    ...
    # ── 7. Navigation links are now dynamically generated by main.js ──────
    # No need to inject static prev/next links; main.js reads index.html
    # and calculates navigation at runtime based on article dates.
...
# Navigation links are now dynamically generated by main.js at runtime.
